...
def test_ranking_prompt_carries_safe_context_framing():
    # The --cross-rank ranking pass builds a fresh prompt from the raw
    # question; with safe_context on, the defensive framing must persist
    # into it (it is the most refusal-prone request of the run).
    ...
    with patch.object(
        ...
        adapters_module, "run_participant", side_effect=fake_run_participant
    ):
        asyncio.run(
            orch_module.execute_council(
                participants=["a", "b"],
    ...
    assert ranking_prompts, "ranking pass did not run"
    for p in ranking_prompts:
        assert SAFE_CONTEXT_DIRECTIVE in p
